...
class RunTestFactory:
    ...
    def createTest(self, test: Test, prefix: str, tool: str, category: str):
        if tool == 'MPIRMA':
            ...
            return RunMUSTTest(test, os.path.join(prefix, tool, category), f"apptainer run --cleanenv {apptainer_dir}/mustrma.sif")
        elif tool == 'MUST-TSan':
            return RunMUSTTSanTest(test, os.path.join(prefix, tool, category), f"apptainer run --cleanenv {apptainer_dir}/must.sif")
