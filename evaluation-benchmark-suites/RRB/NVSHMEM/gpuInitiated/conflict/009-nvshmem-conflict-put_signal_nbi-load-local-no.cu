...
__global__ void nvshmem_kernel(int *remote, int *localbuf, uint64_t *psn_sig_addr) {
    // Initialize memory
    *remote = 0;
    *localbuf = 1;
    *psn_sig_addr = 0;
...
int main(int argc, char **argv) {
    ...
    void *args[] = {remote_d, localbuf_d, psn_sig_addr_d};  // Kernel arguments
    dim3 blocks(1);
    dim3 threads(1);
