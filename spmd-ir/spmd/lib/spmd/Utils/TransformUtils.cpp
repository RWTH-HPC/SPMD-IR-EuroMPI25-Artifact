...
#include "spmd/Utils/TransformUtils.h"
...
void spmd::getUniqueSymbolName(std::string &newName,
...
bool spmd::isLinkonceODR(FunctionOpInterface func) {
...
void spmd::privatize(FunctionOpInterface func) {
  func->setAttr(
      linkageAttrName,
...
bool spmd::isTailCall(CallOpInterface call) {
...
// unsigned spmd::getGridDimension(FunctionOpInterface func) {
...
// SmallVector<Value> spmd::getThreadVector(FunctionOpInterface funcOp,
//                                               DataFlowSolver &solver) {
...
// Optional<Value> spmd::getAccessorUsedByOperation(const Operation &op) {
...
std::optional<APInt> spmd::getConstIntegerValue(Value val,
                                                     DataFlowSolver &solver) {
...
template <typename T> SetVector<T> spmd::getParentsOfType(Block &block) {
  SetVector<T> res;
...
template <typename T>
SetVector<T> spmd::getOperationsOfType(FunctionOpInterface funcOp) {
  SetVector<T> res;
...
//   if (!callers)
//     return;
//   llvm::transform(callers.getAsRange<FlatSymbolRefAttr>(),
//                   std::back_inserter(kernels),
//                   [this](FlatSymbolRefAttr symbol) {
//                     return ST.lookup<gpu::GPUFuncOp>(symbol.getValue());
//                   });
// }

// llvm::SmallSet<FunctionOpInterface, 4>
// FunctionKernelInfo::getKernelFuncObjFunctions(gpu::GPUFuncOp kernel) {
...
//                        [kernelName](FlatSymbolRefAttr symbol) {
//                          return symbol.getValue() == kernelName;
//                        })) {
...
std::optional<IfCondition> IfCondition::getCondition(Operation *op) {
  return TypeSwitch<Operation *, std::optional<IfCondition>>(op)
      ...
      .Case<scf::IndexSwitchOp>([](auto switchOp) {
        return IfCondition(switchOp.getArg());
      })
