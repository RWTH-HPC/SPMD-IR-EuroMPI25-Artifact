...
void UniformityAnalysis::setToEntryState(UniformityLattice *lattice) {
  ...
  if (Region *region = lattice->getAnchor().getParentRegion()) {
    ...
    // The arguments of the main function can be non-uniform, therefore unkown and does not need to be handled.
  //   else if (auto funcOp = dyn_cast<func::FuncOp>(region->getParentOp())) {
  //     if (funcOp.getSymName().equals("main")) {
  //       for (Value arg : funcOp.front().getArguments()) {
  //         UniformityLattice *lattice = getLatticeElement(arg);
  //         propagateIfChanged(lattice, lattice->join(Uniformity::getUnkown()));
  //       }
  //       return;
  //     }
  //   }
...
LogicalResult UniformityAnalysis::visitOperation(
  ...
  if (op->hasTrait<spmd::ResultsNonUniform>()) {
    LLVM_DEBUG(llvm::dbgs().indent(2)
               << "Operation yields non-uniform result(s)\n");
    propagateAllIfChanged(results, Uniformity::getNonUniform());
    return success();
  }

  // If any operand uniformity is unknown/non-uniform the result(s) are also
  // unknown/non-uniform.  
  //Edit: Changed the order to first checking for non-uniformity as this property is stronger than unknown
  ...
  if (anyOfUniformityIs(operands, Uniformity::Kind::Unknown)) {
    ...
    propagateAllIfChanged(results, Uniformity::getUnknown());
    return success();
  }
  ...
  if (isMemoryEffectFree(op)) {
    ...
    propagateAllIfChanged(results, Uniformity::getUniform());
    return success();
  }

  analyzeMemoryEffects(op, operands, results);
  return success();
}
...
void UniformityAnalysis::visitNonControlFlowArguments(
  ...
  if (auto loop = dyn_cast<LoopLikeOpInterface>(op)) {
    if (auto uniformity = getInductionVariableUniformity(loop)) {
      propagateAllIfChanged(*loop.getSingleInductionVar(), *uniformity);
      return;
    } 
  }
...
void UniformityAnalysis::analyzeMemoryEffects(
  ...
  for (const auto &EI : effects) {
    ...
    if (!getOrCreateFor<ReachingDefinition>(getProgramPointBefore(op), getProgramPointBefore(op)))
      return;

    // Get the reaching def. and potential reaching def. of the value 'val'
    // and analyze them to determine its uniformity.
    const ReachingDefinition *rdef = solver.lookupState<ReachingDefinition>(getProgramPointBefore(op));
...
SmallVector<IfCondition> UniformityAnalysis::collectBranchConditions(
  ...
  for (const Definition &mod : mods) {
    ...
    for (RegionBranchOpInterface branchOp : enclosingBranches) {
      if (isa<affine::AffineForOp, scf::ForOp, scf::WhileOp>(branchOp))
        continue;
  ...
  if (!uniformityIsKnown) {
    auto getOrCreateLatticeFor = [&](Value val) {
      ...
      if (lattice->getValue().isUninitialized())
        getOrCreateFor<UniformityLattice>(getProgramPointBefore(op), val);
    };
  ...
  if (!uniformityIsKnown) {
    for (const Definition &def : mods) {
      ...
      for (Value operand : defOp->getOperands()) {
        ...
        if (lattice->getValue().isUninitialized())
          getOrCreateFor<UniformityLattice>(getProgramPointBefore(op), operand);
      }
    }
  }
...
bool UniformityAnalysis::anyModifierUniformityIs(
  ...
  return llvm::any_of(mods, [&](const Definition &def) {
    ...
    // handle spmd collectives, P2Ps, RMA, non-blocking sync
    if (def.getOperation()->hasTrait<spmd::Collective>() || def.getOperation()->hasTrait<spmd::P2P>() 
        || def.getOperation()->hasTrait<spmd::NonBlockingSync>() || def.getOperation()->hasTrait<spmd::RMA>()) {
      return anyOfUniformityIs(def.getOperation()->getOperands(), kind);
    }
    // Handle a concrete definition.
    return TypeSwitch<Operation *, bool>(def.getOperation())
        .Case<memref::AllocaOp, LLVM::AllocaOp, spmd::MallocOp>(
        ...
        .Case<memref::StoreOp, affine::AffineStoreOp, /* sycl::SYCLConstructorOp,
              ...
              sycl::SYCLNDRangeConstructorOp,*/ LLVM::StoreOp, LLVM::MemsetOp, spmd::MemsetOp, spmd::MemcpyOp,
              LLVM::MemcpyOp>(
