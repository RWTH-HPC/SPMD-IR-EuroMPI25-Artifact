...
Value getNewWinMPI(Value winArg, IRMapping &valueMapping){
  Value newWinArg;
  Value winArgAddr = getAddress(winArg, valueMapping);
  if (valueMapping.contains(winArgAddr)) {
    newWinArg = valueMapping.lookup(winArgAddr);
  } 
  else {
...
bool convertApiCallMPI(LLVM::CallOp &llvmCallOp, int modelGroup) {
  ...
  if (*calleeStrRef == "MPI_Init") {
    ...
    valueMapping.map(getAddress(winArg, valueMapping), newWin);
    handleErrorCode(llvmCallOp.getOperation(), newOp->getResult(newOp->getNumResults()-1));
    llvmCallOp->erase();
    return true;
  } 
    ...
    Value newWin = getNewWinMPI(winArg, valueMapping);
