...
BEGIN_MBB_TESTS
  $ mpirun -np 2 ${EXE}
  | OK
  | Correct-lockunlock-rget_accumulate_rget
END_MBB_TESTS
//////////////////////       End of MBI headers        /////////////////// */
...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    MPI_Rget_accumulate(buf, 10, MPI_INT, int_0, 10, MPI_INT, 1, 0, 10, MPI_INT,
                        MPI_SUM, mpi_win_0, &mpi_request_0);
    MPI_Win_unlock(1, mpi_win_0);
    MPI_Rget(buf, 10, MPI_INT, 1, 10, 10, MPI_INT, mpi_win_0, &mpi_request_1);
