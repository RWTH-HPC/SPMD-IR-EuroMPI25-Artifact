...
__global__ void nvshmem_kernel(int *remote, int *localbuf, int *remote_midp, int *localbuf_midp) {
    ...
    if (my_pe == 0) {
        nvshmem_int_get_nbi(localbuf, remote, ARR_SIZE / 4, 1);
...
int main(int argc, char **argv) {
    ...
    int *remote_midp_d = remote_d + ARR_SIZE / 2;
    int *localbuf_midp_d = localbuf_d + ARR_SIZE / 2;
    ...
    void *args[] = {remote_d, localbuf_d, remote_midp_d, localbuf_midp_d};  // Kernel arguments
    dim3 blocks(1);
    dim3 threads(1);
