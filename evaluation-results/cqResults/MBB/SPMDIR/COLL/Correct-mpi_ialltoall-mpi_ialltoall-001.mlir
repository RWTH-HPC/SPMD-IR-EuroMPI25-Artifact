...
#di_file = #llvm.di_file<"cq-results-20250731-154348/MBB/SPMDIR/COLL/Correct-mpi_ialltoall-mpi_ialltoall-001.c" in "/rwthfs/rz/cluster/work/rwth1282/spmdir/spmd/externals/data-race-detection-benchmark-suite">
...
module attributes {dlti.dl_spec = #dlti.dl_spec<i128 = dense<128> : vector<2xi64>, f80 = dense<128> : vector<2xi64>, i1 = dense<8> : vector<2xi64>, !llvm.ptr = dense<64> : vector<4xi64>, i16 = dense<16> : vector<2xi64>, i8 = dense<8> : vector<2xi64>, f16 = dense<16> : vector<2xi64>, f64 = dense<64> : vector<2xi64>, i32 = dense<32> : vector<2xi64>, !llvm.ptr<271> = dense<32> : vector<4xi64>, !llvm.ptr<270> = dense<32> : vector<4xi64>, i64 = dense<64> : vector<2xi64>, f128 = dense<128> : vector<2xi64>, !llvm.ptr<272> = dense<64> : vector<4xi64>, "dlti.stack_alignment" = 128 : i64, "dlti.endianness" = "little">, llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", llvm.ident = "clang version 20.0.0git (https://github.com/burakSemih/llvm-project.git a5b8f53f3db10b0ff5451335362b36a25139a3f8)", llvm.target_triple = "x86_64-unknown-linux-gnu", spmd.executionKind = "All"} {
  ...
  llvm.func local_unnamed_addr @main(%arg0: i32 {llvm.noundef}, %arg1: !llvm.ptr {llvm.noundef}) -> (i32 {llvm.noundef}) attributes {no_unwind, passthrough = [["uwtable", "2"], ["min-legal-vector-width", "0"], ["no-trapping-math", "true"], ["stack-protector-buffer-size", "8"], ["target-cpu", "x86-64"]], spmd.executionKind = "All", target_cpu = "x86-64", target_features = #llvm.target_features<["+cmov", "+cx8", "+fxsr", "+mmx", "+sse", "+sse2", "+x87"]>, tune_cpu = "generic"} {
    ...
    %11 = arith.muli %size, %c10_i32 {spmd.executionKind = "All"} : i32
    %12 = arith.extsi %11 {spmd.executionKind = "All"} : i32 to i64
    %13 = llvm.call @calloc(%12, %c4_i64) {memory_effects = #llvm.memory_effects<other = none, argMem = none, inaccessibleMem = readwrite>, no_unwind, spmd.executionKind = "All", will_return} : (i64, i64) -> !llvm.ptr
    %14 = llvm.call @calloc(%12, %c4_i64) {memory_effects = #llvm.memory_effects<other = none, argMem = none, inaccessibleMem = readwrite>, no_unwind, spmd.executionKind = "All", will_return} : (i64, i64) -> !llvm.ptr
    %15 = arith.cmpi eq, %rank, %c0_i32 {spmd.executionKind = "All"} : i32
    scf.if %15 {
      %req, %error_2 = "spmd.alltoall"(%2, %13, %c10_i64, %3, %14, %c10_i64, %3) <{isBlocking = false, usedModel = 0 : i32}> {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (!spmd.comm, !llvm.ptr, i64, !spmd.datatype, !llvm.ptr, i64, !spmd.datatype) -> (!spmd.req, !spmd.error)
    ...
    scf.if %16 {
      ...
      %req, %error_2 = "spmd.alltoall"(%2, %13, %c10_i64, %3, %14, %c10_i64, %3) <{isBlocking = false, usedModel = 0 : i32}> {spmd.executedNotBy = array<i32: 0>, spmd.executionKind = "AllBut"} : (!spmd.comm, !llvm.ptr, i64, !spmd.datatype, !llvm.ptr, i64, !spmd.datatype) -> (!spmd.req, !spmd.error)
      memref.store %req, %alloca[%c0] {spmd.executedNotBy = array<i32: 0>, spmd.executionKind = "AllBut"} : memref<1x!spmd.req>
    } {spmd.executionKind = "All", spmd.isMultiValued = true}
    %17 = memref.load %alloca[%c0] {spmd.executionKind = "All"} : memref<1x!spmd.req>
    %error_1 = "spmd.wait"(%17) <{usedModel = 0 : i32}> {spmd.executionKind = "All"} : (!spmd.req) -> !spmd.error
    ...
    llvm.call @free(%14) {memory_effects = #llvm.memory_effects<other = none, argMem = readwrite, inaccessibleMem = readwrite>, no_unwind, spmd.executionKind = "All", will_return} : (!llvm.ptr) -> ()
