...
#di_file = #llvm.di_file<"cq-results-20250731-154348/MBB/SPMDIR/RMA/Correct-fence-rget_rget_accumulate-001.c" in "/rwthfs/rz/cluster/work/rwth1282/spmdir/spmd/externals/data-race-detection-benchmark-suite">
...
#di_global_variable = #llvm.di_global_variable<file = #di_file, line = 66, type = #di_composite_type, isLocalToUnit = true, isDefined = true>
...
module attributes {dlti.dl_spec = #dlti.dl_spec<!llvm.ptr = dense<64> : vector<4xi64>, f80 = dense<128> : vector<2xi64>, i64 = dense<64> : vector<2xi64>, i128 = dense<128> : vector<2xi64>, !llvm.ptr<272> = dense<64> : vector<4xi64>, !llvm.ptr<271> = dense<32> : vector<4xi64>, !llvm.ptr<270> = dense<32> : vector<4xi64>, f64 = dense<64> : vector<2xi64>, f128 = dense<128> : vector<2xi64>, f16 = dense<16> : vector<2xi64>, i32 = dense<32> : vector<2xi64>, i16 = dense<16> : vector<2xi64>, i8 = dense<8> : vector<2xi64>, i1 = dense<8> : vector<2xi64>, "dlti.stack_alignment" = 128 : i64, "dlti.endianness" = "little">, llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128", llvm.ident = "clang version 20.0.0git (https://github.com/burakSemih/llvm-project.git a5b8f53f3db10b0ff5451335362b36a25139a3f8)", llvm.target_triple = "x86_64-unknown-linux-gnu", spmd.executionKind = "All"} {
  ...
  llvm.func local_unnamed_addr @main(%arg0: i32 {llvm.noundef}, %arg1: !llvm.ptr {llvm.noundef}) -> (i32 {llvm.noundef}) attributes {no_unwind, passthrough = [["uwtable", "2"], ["min-legal-vector-width", "0"], ["no-trapping-math", "true"], ["stack-protector-buffer-size", "8"], ["target-cpu", "x86-64"]], spmd.executionKind = "All", target_cpu = "x86-64", target_features = #llvm.target_features<["+cmov", "+cx8", "+fxsr", "+mmx", "+sse", "+sse2", "+x87"]>, tune_cpu = "generic"} {
    ...
    llvm.store %c738197504_i32, %9 {alignment = 4 : i64, spmd.executionKind = "All", tbaa = [#tbaa_tag]} : i32, !llvm.ptr
    llvm.intr.lifetime.start 4, %10 {spmd.executionKind = "All"} : !llvm.ptr
    llvm.store %c738197504_i32, %10 {alignment = 4 : i64, spmd.executionKind = "All", tbaa = [#tbaa_tag]} : i32, !llvm.ptr
    ...
    %13 = arith.cmpi slt, %size, %c2_i32 {spmd.executionKind = "All"} : i32
    scf.if %13 {
      %23 = llvm.call @puts(%0) {no_unwind, spmd.executionKind = "All"} : (!llvm.ptr) -> i32
    } {spmd.executionKind = "All", spmd.isMultiValued = false}
    llvm.intr.lifetime.start 4, %11 {spmd.executionKind = "All"} : !llvm.ptr
    %14 = llvm.call @calloc(%c20_i64, %c4_i64) {memory_effects = #llvm.memory_effects<other = none, argMem = none, inaccessibleMem = readwrite>, no_unwind, spmd.executionKind = "All", will_return} : (i64, i64) -> !llvm.ptr
    %win, %error_2 = "spmd.winCreate"(%2, %14, %c80_i64, %c4_i32) <{usedModel = 0 : i32}> {spmd.executionKind = "All"} : (!spmd.comm, !llvm.ptr, i64, i32) -> (!spmd.win, !spmd.error)
    ...
    scf.if %16 {
      %23 = llvm.call @calloc(%c10_i64, %c4_i64) {memory_effects = #llvm.memory_effects<other = none, argMem = none, inaccessibleMem = readwrite>, no_unwind, spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static", will_return} : (i64, i64) -> !llvm.ptr
      %req, %error_3 = "spmd.get"(%23, %c10_i64, %3, %c1_i64, %c1_i32, %win, %c0_i64, %c10_i64, %3, %c1_i64) <{isAtomic = true, isBlocking = false, usedModel = 0 : i32}> {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (!llvm.ptr, i64, !spmd.datatype, i64, i32, !spmd.win, i64, i64, !spmd.datatype, i64) -> (!spmd.req, !spmd.error)
      memref.store %req, %alloca[%c0] {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : memref<1x!spmd.req>
      %24 = "spmd.fence"(%win, %c0_i32) <{usedModel = 0 : i32}> {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (!spmd.win, i32) -> !spmd.error
      %25 = llvm.call @calloc(%c10_i64, %c4_i64) {memory_effects = #llvm.memory_effects<other = none, argMem = none, inaccessibleMem = readwrite>, no_unwind, spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static", will_return} : (i64, i64) -> !llvm.ptr
      %req_4, %error_5 = "spmd.getAccumulate"(%23, %c10_i64, %3, %c1_i64, %25, %c10_i64, %3, %c1_i64, %c0_i32, %win, %c0_i64, %c10_i64, %3, %c1_i64, %4) <{isAtomic = true, isBlocking = false, usedModel = 0 : i32}> {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (!llvm.ptr, i64, !spmd.datatype, i64, !llvm.ptr, i64, !spmd.datatype, i64, i32, !spmd.win, i64, i64, !spmd.datatype, i64, !spmd.op) -> (!spmd.req, !spmd.error)
      memref.store %req_4, %alloca_0[%c0] {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : memref<1x!spmd.req>
    } {spmd.executionKind = "All", spmd.isMultiValued = true}
    %17 = arith.cmpi eq, %rank, %c1_i32 {spmd.executionKind = "All"} : i32
    scf.if %17 {
      %23 = "spmd.fence"(%win, %c0_i32) <{usedModel = 0 : i32}> {spmd.executedBy = array<i32: 1>, spmd.executionKind = "Static"} : (!spmd.win, i32) -> !spmd.error
    } {spmd.executionKind = "All", spmd.isMultiValued = true}
    %18 = "spmd.fence"(%win, %c0_i32) <{usedModel = 0 : i32}> {spmd.executionKind = "All"} : (!spmd.win, i32) -> !spmd.error
    %19 = arith.cmpi eq, %rank, %c0_i32 {spmd.executionKind = "All"} : i32
    scf.if %19 {
      %23 = memref.load %alloca[%c0] {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : memref<1x!spmd.req>
      %error_3 = "spmd.wait"(%23) <{usedModel = 0 : i32}> {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (!spmd.req) -> !spmd.error
      %24 = memref.load %alloca_0[%c0] {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : memref<1x!spmd.req>
      %error_4 = "spmd.wait"(%24) <{usedModel = 0 : i32}> {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (!spmd.req) -> !spmd.error
    } {spmd.executionKind = "All", spmd.isMultiValued = true}
    %20 = "spmd.winFree"(%win) <{usedModel = 0 : i32}> {spmd.executionKind = "All"} : (!spmd.win) -> !spmd.error
    %21 = "spmd.finalize"() <{usedModel = 0 : i32}> {spmd.executionKind = "All"} : () -> !spmd.error
    %22 = llvm.call @printf(%1, %rank) vararg(!llvm.func<i32 (ptr, ...)>) {no_unwind, spmd.executionKind = "All"} : (!llvm.ptr, i32) -> i32
