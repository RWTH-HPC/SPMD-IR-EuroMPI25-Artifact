...
__attribute__((noinline)) void deeeeeeeeep(int* rem_ptr_d, int* lbuf_ptr_d) { nvshmem_int_put_nbi(rem_ptr_d, lbuf_ptr_d, 1, 1); }
...
void rank0(int* rem_ptr_d, int* lbuf_ptr_d)
{
    ...
    cudaMemcpy(&lbuf, lbuf_ptr_d, sizeof(int), cudaMemcpyDeviceToHost);
    printf("lbuf is %d\n", lbuf);
}
